...
EOS;
    ...
    public function get_development_asset(object $manifest, string $entry, array $options) : ?array
    {
        ...
        $dependencies = \array_merge([self::VITE_CLIENT_SCRIPT_HANDLE], $options['dependencies']);
        $src = $this->generate_development_asset_src($manifest, $entry);
        $assets = ['scripts' => [$vite_client_script, $src], 'styles' => $options['css-dependencies']];
        return $assets;
        // return [
    ...
    public function _get_assets(string $manifest_dir, string $entry, array $options)
    {
        ...
        $options = $this->parse_options($options);
        return $manifest->is_dev ? $this->get_development_asset($manifest, $entry, $options) : $this->get_production_asset($manifest, $entry, $options);
    }
    /**
     * Enqueue asset
     *
